import React from 'react';
import { useAuth } from '../contexts/AuthContext';

const ProfilePage = () => {
  const { username } = useAuth();
  const randomImageUrl = 'data:image/png;base64,';

  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="w-full max-w-5xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
        <div className="bg-gradient-to-r from-gray-700 to-gray-900 p-8 rounded-t-xl flex items-center justify-center flex-col">
          <img
            className="h-48 w-48 rounded-full border-4 border-white mb-4"
            src={randomImageUrl}
            alt="Profile"
          />
          <h2 className="text-center text-3xl font-semibold text-white">
            {username || 'Guest'}
          </h2>
          <p className="text-center text-white">New York, United States</p>
        </div>
        <div className="p-8">
          <p className="text-gray-700 text-center text-xl mb-2">
            Loyal Customer - Member since 2021
          </p>
          <div className="flex justify-around text-gray-700 text-center mb-8">
            <div>
              <span className="block text-2xl font-bold">15</span>
              <span>Orders</span>
            </div>
            <div>
              <span className="block text-2xl font-bold">5</span>
              <span>Wishlist Items</span>
            </div>
            <div>
              <span className="block text-2xl font-bold">3</span>
              <span>Reviews</span>
            </div>
          </div>
          <div className="flex justify-center space-x-4 mb-4">
            <button className="px-6 py-3 bg-gray-800 text-white rounded-full shadow-md hover:bg-gray-700">
              View Orders
            </button>
            <button className="px-6 py-3 bg-gray-800 text-white rounded-full shadow-md hover:bg-gray-700">
              Wishlist
            </button>
          </div>
          <button className="w-full bg-gray-800 text-white px-6 py-3 rounded-full shadow-md hover:bg-gray-700">
            Edit Profile
          </button>
        </div>
      </div>
    </div>
  );
};

export default ProfilePage;
